...
# Load a test image from disk (you can replace 'test_image.jpg' with your actual image path)
...
results = model(img)  # Pass the image to the model for object detection
...
# Draw bounding boxes and labels on the image for knives and guns
...
# Show the image with the detections (in a window)
cv2.imshow("YOLOv5 Detection (Knives & Guns)", img)

# Save the image with bounding boxes drawn (optional)
